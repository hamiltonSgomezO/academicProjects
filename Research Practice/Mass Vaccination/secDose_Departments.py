...
C_f = pd.read_csv('C:/Users/USUARIO/Documents/Universidad_EAFIT/7_Septimo_Semestre/Practica _Investigativa_2/Primera_implementación/C_10f.csv',header=None,dtype='int32',delimiter=';')
...
w = pd.read_csv('C:/Users/USUARIO/Documents/Universidad_EAFIT/7_Septimo_Semestre/Practica _Investigativa_2/Primera_implementación/W_it_10f.csv',header=None,dtype='int',delimiter=';')
...
Q = pd.read_csv('C:/Users/USUARIO/Documents/Universidad_EAFIT/7_Septimo_Semestre/Practica _Investigativa_2/Primera_implementación/Q_ij_10f.csv',header=None,dtype='int32',delimiter=';')
...
F = pd.read_csv('C:/Users/USUARIO/Documents/Universidad_EAFIT/7_Septimo_Semestre/Practica _Investigativa_2/Primera_implementación/F_ki_10f.csv',header=None,dtype='int32',delimiter=';')
...
for i in range(1,dep+1):
    for t in range(1,per+1):
        for j in range(1,stage+1):
            model.addConstr((L[t,j] >= (quicksum(D[k,i,t,j,2]  for k in range(1,vacc+1)) + A[5,i,t,j])/Q[i-1][j-1]), "Constraints " + str(cont))
            cont += 1
...
for i in range(1,dep+1):
    for t in range(1,per+1):
        for j in range(1,stage+1):
            model.addConstr((T[t,j] <= (quicksum(D[k,i,t,j,2]  for k in range(1,vacc+1)) + A[5,i,t,j])/Q[i-1][j-1]), "Constraints " + str(cont))
            cont += 1
...
#constraint 15
    for i in range(1,dep+1):
        for t in range(1,per+1):
            for j in range(1,stage):
                model.addConstr((D[5,i,t,j,2]  == 0), "Constraints " + str(cont))
                cont += 1
